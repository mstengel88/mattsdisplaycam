# !!!Warning!!!
Older Raspberry Pi OS releases use the original omxplayer backend. Raspberry Pi OS / Debian Trixie no longer ships omxplayer, so displaycameras now supports an mpv backend for Trixie installs. The installer chooses the backend automatically; you can override it with `player_backend=omxplayer` or `player_backend=mpv` in `/etc/displaycameras/displaycameras.conf`.
# Project Description
displaycameras is a set of scripts run as a service on RaspberryPi hardware to locally display RTSP streams from Ubiquiti security camera systems. It uses omxplayer on legacy Raspberry Pi OS releases or mpv on Trixie to play each configured feed in a grid of "windows" into which you divide your display. It monitors startup, watchdog, and repair state to maximize predictable, reliable performance. It will optionally auto-detect screen resolution (and apply customized configuration) in case RPis don't have reliably consistent displays (e.g., mobile use). It is capable of displaying more feeds than there are visible windows in your display grid by rendering some feeds off screen and rotating feeds through window positions (on and off screen) in order to visibly display all feeds over a reasonable period of time.
...
`sudo ./install.sh --remote-access`

This installs and enables `displaycameras-vnc.service`, which runs `x11vnc` on the local display and binds it to localhost on port 5900. Connect through SSH from another computer:

`ssh -L 5900:localhost:5900 pi@<raspberry-pi-address>`

Then point a VNC viewer at `localhost:5900`. Keeping VNC bound to localhost avoids exposing an unauthenticated VNC port directly on your network.

The optional VNC service expects the camera display to be running in an X11 session on display `:0`. If your Trixie desktop is configured for Wayland only, switch the Pi to X11 before enabling this service.
...
3. Remove the files:
	...
	e. If remote access was installed, remove it with `sudo systemctl disable --now displaycameras-vnc.service && sudo rm /etc/systemd/system/displaycameras-vnc.service`
4. Remove the pre-requisites (optional): `for each package in omxplayer mpv socat fbi x11vnc; do sudo apt-get purge $package -y; done`
...
#### Verify mpv will play your feed RTSP URLs
On Trixie, verify feeds with:

`mpv --rtsp-transport=tcp <camera feed URL>`
